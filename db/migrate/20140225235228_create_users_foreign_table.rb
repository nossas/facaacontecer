class CreateUsersForeignTable < ActiveRecord::Migration

  # Creating the foreign table on meurio_accounts server.
  # This way, we can sync users from the project.
  def up
  ...
  # Revert the foreign table creation
  def down
    if Rails.env.production? || Rails.env.staging?
